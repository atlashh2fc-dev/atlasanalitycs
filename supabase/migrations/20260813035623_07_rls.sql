...
create policy ges_categoria_lectura on ges_categoria for select to authenticated using (true);
create policy ges_problema_lectura on ges_problema for select to authenticated using (true);
create policy feriado_lectura on feriado for select to authenticated using (true);

create policy tenant_propio on tenant for select to authenticated using (id = current_tenant_id());
create policy perfil_lectura on perfil for select to authenticated using (tenant_id = current_tenant_id());
create policy perfil_admin_escribe on perfil for all to authenticated
...
begin
  foreach t in array array[
    ...
    execute format($f$
      create policy %1$s_tenant_lectura on %1$I for select to authenticated
        using (tenant_id = current_tenant_id());
    $f$, t);
    execute format($f$
      create policy %1$s_tenant_escritura on %1$I for all to authenticated
        using (tenant_id = current_tenant_id() and es_admin())
        with check (tenant_id = current_tenant_id() and es_admin());
    $f$, t);
  end loop;
...
begin
  foreach t in array array[
    ...
    if t = 'campana' then
      execute $f$
        create policy campana_visible on campana for select to authenticated
          using (id in (select campanas_visibles()));
      $f$;
      execute $f$
        create policy campana_escritura on campana for all to authenticated
          using (tenant_id = current_tenant_id() and es_admin())
          with check (tenant_id = current_tenant_id() and es_admin());
      $f$;
    else
      execute format($f$
        create policy %1$s_campana_visible on %1$I for select to authenticated
          using (tenant_id = current_tenant_id()
                 and (campana_id is null or campana_id in (select campanas_visibles())));
      $f$, t);
      execute format($f$
        create policy %1$s_campana_escritura on %1$I for all to authenticated
          using (tenant_id = current_tenant_id()
...
create policy carga_columna_visible on carga_columna for all to authenticated
...
create policy fila_cruda_visible on fila_cruda for all to authenticated
...
create policy carga_reversion_visible on carga_reversion for all to authenticated
...
create policy venta_asegurado_visible on venta_asegurado for all to authenticated
...
create policy venta_preexistencia_visible on venta_preexistencia for all to authenticated
  using (exists (select 1 from venta_asegurado va where va.id = venta_preexistencia.venta_asegurado_id))
  with check (exists (select 1 from venta_asegurado va where va.id = venta_preexistencia.venta_asegurado_id));

create policy anulacion_visible on anulacion for all to authenticated
...
create policy enlace_visible on enlace_cotizacion_venta for all to authenticated
...
create policy perfil_campana_visible on perfil_campana for select to authenticated
  using (exists (select 1 from perfil p where p.id = perfil_campana.perfil_id
                   and p.tenant_id = current_tenant_id()));

create policy perfil_campana_admin on perfil_campana for all to authenticated
  using (es_admin() and exists (select 1 from perfil p where p.id = perfil_campana.perfil_id
                                  and p.tenant_id = current_tenant_id()))
  with check (es_admin() and exists (select 1 from perfil p where p.id = perfil_campana.perfil_id
                                       and p.tenant_id = current_tenant_id()));

create policy ejecutivo_campana_visible on ejecutivo_campana for all to authenticated
