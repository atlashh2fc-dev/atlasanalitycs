...
-- su propio costo, y cuánto para llegar a su meta.
--
-- Tres decisiones:
--
...
--    error de cálculo.
--
-- 2. Punto de equilibrio individual. Es el costo fijo dividido por el
--    margen de contribución por asegurado, a su mix y su tramo de
--    comisión actuales. Si cambia de tramo el número se mueve, y por eso
--    la vista también muestra el margen en pesos, que es exacto.
--
...
-- Qué parte del periodo ya pasó, para el ritmo esperado.
avance_periodo as (
  select case
...
eco as (
  select * from economia_ejecutivo(p_desde, p_hasta, p_campana)
),
com as (
  select
    ejecutivo_id,
...
-- Los que participan del reparto de la meta del equipo.
activos as (
...
select
  ...
  x.meta_asignada,
  x.meta_es_propia,
  round(x.asegurados::numeric / nullif(x.meta_asignada, 0) * 100, 1) as cumplimiento_pct,
  round(x.meta_asignada * (select t from avance_periodo), 1) as ritmo_esperado,
  case when (select t from avance_periodo) > 0
       then round(x.asegurados / (select t from avance_periodo), 1) end as proyeccion,
  x.ingreso_clp,
  x.costo_fijo_clp,
  x.costo_variable_clp,
  x.costo_fijo_clp + x.costo_variable_clp as costo_total_clp,
  x.ingreso_clp - x.costo_fijo_clp - x.costo_variable_clp as margen_clp,
  -- Costo fijo dividido por el margen de contribución de cada asegurado.
  case
    when x.asegurados > 0
     and (x.ingreso_clp - x.costo_variable_clp) > 0
    then round(
      x.costo_fijo_clp
      / ((x.ingreso_clp - x.costo_variable_clp) / x.asegurados), 1)
  end as equilibrio_aseg,
  case
...
comment on function control_ejecutivo is
  'Una fila por ejecutivo con su meta asignada, su ritmo y su punto de
   equilibrio individual. La meta se reparte por jornada cuando no hay
   una meta individual cargada.';;
