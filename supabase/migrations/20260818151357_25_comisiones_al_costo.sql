...
escalonadas as (
  select
    ...
    coalesce(sum(ce.beneficiarios), 0)::numeric as beneficiarios
...
union all
select 'Comisiones'::text, 'por beneficiario, por tramo'::text,
       (select beneficiarios from escalonadas),
       round((select comisiones from escalonadas)
             + coalesce((select plana from remuneraciones), 0), 0)
union all
select 'Bonos'::text, 'por ventas alcanzadas'::text,
       null::numeric,
