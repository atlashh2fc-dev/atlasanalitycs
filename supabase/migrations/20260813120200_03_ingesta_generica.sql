-- =====================================================================
-- Atlas Analytics — 03. Motor de ingesta genérico
--   Esta es la capa que hace que Atlas sirva para CUALQUIER Excel,
--   no sólo para contact center.
-- =====================================================================

-- ---------------------------------------------------------------------
-- Dataset: una "familia" de archivos que el usuario carga
-- periódicamente (ej.: "Ventas diarias", "Base UCC CM").
-- ---------------------------------------------------------------------
...
create trigger t_dataset_updated before update on dataset
  for each row execute function tg_set_updated_at();

-- ---------------------------------------------------------------------
-- Carga: un archivo+hoja concreto subido en un momento
--   'metadatos' guarda lo extraído del nombre de hoja/archivo (P9):
--   {"linea":"CM","dia_carga":5,"mes":"agosto"}
-- ---------------------------------------------------------------------
create table carga (
  id             uuid primary key default gen_random_uuid(),
  tenant_id      uuid not null references tenant(id) on delete cascade,
...
-- Detecta recarga del mismo archivo: mismo hash = mismo contenido
create index on carga (tenant_id, hash_sha256);
create trigger t_carga_updated before update on carga
  for each row execute function tg_set_updated_at();

-- ---------------------------------------------------------------------
-- Perfilado de columnas
--   El perfilador clasifica por CONTENIDO (P7): una columna llamada
--   'deded' cuyo contenido son tramos etarios se clasifica como
--   'categoria' con rol_semantico = 'tramo_etario'.
-- ---------------------------------------------------------------------
...
comment on column carga_columna.varianza_cero is
  ...
  '(en los datos de muestra: id_canal, IdSubCanal, total, IdEstado, Acepta*).';

-- ---------------------------------------------------------------------
-- Filas crudas
--   Todo lo cargado se conserva tal cual. Las tablas canónicas se
--   derivan de acá, así que una corrección de mapeo no obliga a
--   pedirle el archivo de nuevo al usuario.
-- ---------------------------------------------------------------------
...
--   'firma' es el conjunto ordenado de nombres normalizados de columna.
--   Si un archivo nuevo trae la misma firma, se aplica la plantilla sin
--   preguntar. Si trae una firma parecida, se propone y se pide
--   confirmación.
-- ---------------------------------------------------------------------
...
-- array_to_string no es inmutable, así que la firma se calcula por trigger
...
create trigger t_plantilla_updated before update on plantilla_mapeo
  for each row execute function tg_set_updated_at();

comment on column plantilla_mapeo.config_matriz is
  'Para modo matriz (P11 - hojas de Asistencia): fila donde están las '
  'fechas, columna de la entidad, columna de jornada, mapa de marcas. '
  'El motor hace unpivot a formato largo con esto.';

-- ---------------------------------------------------------------------
-- Diccionario de sinónimos: el sistema aprende
--   Cada mapeo confirmado por un humano suma una confirmación. Con eso
--   'ultima_agenda', 'Última agenda' y 'Ultima_Agenda' convergen al
--   mismo rol semántico sin intervención futura.
-- ---------------------------------------------------------------------
...
create unique index on sinonimo_columna
  (coalesce(tenant_id, '00000000-0000-0000-0000-000000000000'::uuid),
   nombre_normalizado, rol_semantico);
create trigger t_sinonimo_updated before update on sinonimo_columna
  for each row execute function tg_set_updated_at();

-- ---------------------------------------------------------------------
-- Semilla de sinónimos globales, derivada de los archivos reales
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Reversión de cargas
--   "La última carga manda", pero nada se pisa en silencio: cada carga
--   es reversible y deja rastro en auditoria.
-- ---------------------------------------------------------------------
