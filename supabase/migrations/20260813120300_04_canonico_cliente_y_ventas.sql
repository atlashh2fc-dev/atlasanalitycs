-- =====================================================================
-- Atlas Analytics — 04. Modelo canónico: cliente, gestión y venta
-- =====================================================================

-- ---------------------------------------------------------------------
-- Cliente: la llave maestra es el RUT del cliente final contactado
-- ---------------------------------------------------------------------
...
create trigger t_cliente_updated before update on cliente
  for each row execute function tg_set_updated_at();

-- Normaliza RUT, email y teléfono en la puerta de entrada
...
--   'cuenta_como_contacto' e 'incluye_en_conversion' permiten calcular
--   contactabilidad y conversión sin hardcodear strings.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Gestión: el hecho central del contact center
--   Un cliente tiene N gestiones en el tiempo. El estado NO vive en el
--   cliente: se deriva de la última gestión, conservando la trazabilidad.
-- ---------------------------------------------------------------------
...
-- Idempotencia: la misma gestión recargada no se duplica
create unique index on gestion (tenant_id, id_externo) where id_externo is not null;

-- ---------------------------------------------------------------------
-- Cotización
--   Nota: el archivo de cotizaciones NO trae RUT (P1). cliente_id queda
--   nullable y se resuelve por matching probabilístico.
-- ---------------------------------------------------------------------
...
create trigger t_venta_updated before update on venta
  for each row execute function tg_set_updated_at();

comment on column venta.n_asegurados is
  'Titular + cargas. Es la UNIDAD DE LA META (250 CM+CAT / 60 Onco). '
  'Validado contra los archivos: 83 contratos = 104 asegurados.';

-- ---------------------------------------------------------------------
-- Asegurados de la venta
--   Explota los campos concatenados "id | RUT | nombre | fecha nac |
--   preexistencia" de Datos Titular y Datos Beneficiario 1..8 (P5).
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Taxonomía GES/AUGE (MINSAL) — 90 problemas de salud, 10 categorías
--   Las preexistencias NO se enmascaran: se categorizan y agrupan.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Catálogo del checklist de la DPS
--   Cada ítem del formulario se mapea a un problema GES (y por herencia
--   a su categoría). Lo que no calza cae al capítulo CIE-10 de respaldo.
--   Tabla editable: el mapeo no vive en código.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Anulaciones
--   Decisión de negocio: la anulación descuenta en el periodo en que se
--   LEVANTA, no en el periodo de la venta original. Por eso
--   'periodo_descuento' es una columna propia y no se deriva de la venta.
-- ---------------------------------------------------------------------
create table anulacion (
  ...
  tipo              text,
  motivo            text,
  n_asegurados      smallint,
  monto_uf          numeric(12,4),
  origen            text,
  carga_id          uuid references carga(id) on delete set null,
  created_at        timestamptz not null default now(),
...
comment on column anulacion.periodo_descuento is
  'Primer día del mes en que se descuenta. Por defecto = mes de '
  'fecha_anulacion, según decisión de negocio.';

-- ---------------------------------------------------------------------
-- Enlace cotización <-> venta
--   No existe id de cotización en el origen (P1). El enlace se resuelve
--   por email, teléfono y nombre, guardando SIEMPRE el score y el
--   método, para que el dashboard pueda rotular la conversión como
--   estimada y el usuario pueda confirmar a mano.
-- ---------------------------------------------------------------------
