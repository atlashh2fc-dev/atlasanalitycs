-- =====================================================================
-- Atlas Analytics — 02. Tenancy, usuarios y maestros
-- =====================================================================
...
create trigger t_tenant_updated before update on tenant
...
create trigger t_perfil_updated before update on perfil
  for each row execute function tg_set_updated_at();

-- ---------------------------------------------------------------------
-- Funciones de contexto para RLS
--   SECURITY DEFINER y search_path fijo: evitan recursión de políticas
...
create trigger t_campana_updated before update on campana
  for each row execute function tg_set_updated_at();

-- ---------------------------------------------------------------------
-- Acceso de supervisores a campañas
--   Un admin ve todo su tenant; un supervisor sólo sus campañas.
-- ---------------------------------------------------------------------
  ...
  select c.id
   ...
   where c.tenant_id = current_tenant_id()
     and (
       es_admin()
       or exists (select 1
                    from perfil_campana pc
                   where pc.campana_id = c.id
...
-- ---------------------------------------------------------------------
-- Ejecutivo + alias
--   El alias resuelve P2: "Francisca  Valenzuela" y "Francisca Valenzuela"
--   son la misma persona. Todo texto entrante se busca contra
--   alias_normalizado antes de crear un ejecutivo nuevo.
-- ---------------------------------------------------------------------
...
create trigger t_ejecutivo_updated before update on ejecutivo
  for each row execute function tg_set_updated_at();
...
-- Normaliza el alias automáticamente al insertar
...
-- ---------------------------------------------------------------------
-- Producto y agrupación de meta
--   Complementario y Catastrófico comparten meta (250) pero se
--   desglosan en el dashboard. Oncológico básico y total comparten
--   meta (60). De ahí la separación entre 'linea' y 'agrupacion_meta'.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Metas con vigencia temporal
--   ejecutivo_id NULL  -> meta del equipo (caso actual: 250 / 60)
--   ejecutivo_id NOT NULL -> meta individual
-- ---------------------------------------------------------------------
create table meta (
  id              uuid primary key default gen_random_uuid(),
  tenant_id       uuid not null references tenant(id) on delete cascade,
...
create trigger t_meta_updated before update on meta
  for each row execute function tg_set_updated_at();

-- ---------------------------------------------------------------------
-- Diccionario de métricas editable
--   Ningún umbral ni fórmula queda enterrado en código.
-- ---------------------------------------------------------------------
create table metrica_def (
  id               uuid primary key default gen_random_uuid(),
  tenant_id        uuid references tenant(id) on delete cascade, -- null = global Atlas
...
create trigger t_metrica_def_updated before update on metrica_def
  for each row execute function tg_set_updated_at();
