-- =====================================================================
-- Atlas Analytics — 01. Extensiones, tipos y funciones de apoyo
-- =====================================================================
...
--   Resuelve P2 (nombres de ejecutivo con espacios dobles/finales)
--   y P6 (nombres de columna que cambian entre hojas: tildes, guiones,
--   mayúsculas).
-- ---------------------------------------------------------------------
...
comment on function normaliza_texto is
  'Minúsculas, sin tildes, espacios colapsados y recortados. Base de toda conciliación.';
...
--   Permite al perfilador identificar la columna de RUT por CONTENIDO,
--   sin depender de cómo se llame (P7).
-- ---------------------------------------------------------------------
...
begin
  ...
  -- el cuerpo debe ser numérico; sólo el DV puede ser K
  if cuerpo !~ '^[0-9]+$' then
    return null;
  end if;
...
begin
  ...
  if cuerpo = '' or length(cuerpo) > 9 then
    return false;
  end if;
...
comment on function valida_rut is
  'Validación módulo 11. El perfilador la usa sobre una muestra de la columna: '
  'si >90% valida, la columna es RUT sin importar su nombre.';

-- ---------------------------------------------------------------------
-- Teléfono chileno (para el matching cotización <-> venta, P1)
-- ---------------------------------------------------------------------
...
as $$
  select case
           ...
           when length(d) = 8              then '569' || d   -- 12345678
           when length(d) = 9              then '56'  || d   -- 912345678
           when length(d) = 11             then d            -- 56912345678
           else right(d, 11)
         end
...
-- ---------------------------------------------------------------------
-- Tramo etario (los datos traen esto en columnas llamadas 'dede'/'deded')
-- ---------------------------------------------------------------------
