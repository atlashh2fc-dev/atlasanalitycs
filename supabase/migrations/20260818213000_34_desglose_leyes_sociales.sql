...
asegurados_por_ejecutivo as (
  select v.ejecutivo_id, count(va.id)::numeric as asegurados
...
factores as (
  select r.ejecutivo_id, r.factor_leyes
  from remuneracion r
...
remuneraciones as (
  select
    coalesce(sum(r.sueldo_base_clp * (select f from fraccion)), 0) as sueldo,
    coalesce(sum(r.sueldo_base_clp * (select f from fraccion) * (r.factor_leyes - 1)), 0) as leyes_sueldo,
    coalesce(sum(coalesce(a.asegurados, 0) * r.comision_asegurado_clp), 0) as comision_plana,
    coalesce(sum(coalesce(a.asegurados, 0) * r.comision_asegurado_clp * (r.factor_leyes - 1)), 0) as leyes_plana
  from remuneracion r
...
escalonadas as (
  select
    coalesce(sum(ce.comision_clp), 0) as comisiones,
    coalesce(sum(ce.semana_corrida_clp), 0) as semana_corrida,
    coalesce(sum(ce.bonos_clp), 0) as bonos,
    coalesce(sum(
      (ce.comision_clp + ce.semana_corrida_clp + ce.bonos_clp)
      * (coalesce(f.factor_leyes, 1) - 1)
    ), 0) as leyes_variables,
...
select 'Remuneración base'::text, 'mensual'::text, round((select f from fraccion), 4), round((select sueldo from remuneraciones), 0)
union all
select 'Leyes sociales · remuneración'::text, 'mensual'::text, null::numeric, round((select leyes_sueldo from remuneraciones), 0)
union all
select 'Comisiones variables'::text, 'por unidad y tramo'::text, (select unidades from escalonadas),
       round((select comisiones from escalonadas) + (select comision_plana from remuneraciones), 0)
union all
select 'Semana corrida'::text, 'sobre comisión'::text, null::numeric, round((select semana_corrida from escalonadas), 0)
union all
select 'Bonos variables'::text, 'por metas alcanzadas'::text, null::numeric, round((select bonos from escalonadas), 0)
union all
select 'Leyes sociales · variables'::text, 'sobre comisiones y bonos'::text, null::numeric,
       round((select leyes_variables from escalonadas) + (select leyes_plana from remuneraciones), 0)
union all
...
comment on function costos_periodo is
  'Desglosa remuneracion bruta, leyes sociales, comisiones, semana corrida, bonos y operacion sin alterar el costo empresa total.';
