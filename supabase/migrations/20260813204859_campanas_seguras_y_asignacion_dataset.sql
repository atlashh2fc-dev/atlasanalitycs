-- Una FK simple comprueba que la campaña existe, pero no que pertenezca al
-- mismo tenant. Este trigger protege cargas nuevas, ediciones directas y RPCs.
...
begin
  if new.campana_id is not null and not exists (
    ...
    raise exception 'La campaña y el registro deben pertenecer al mismo espacio.';
  end if;
  return new;
end;
$$;

revoke execute on function public.tg_campana_mismo_tenant()
...
-- Crea una asociación consistente entre una base y una campaña después de
-- la carga. La función es SECURITY DEFINER para que la operación completa sea
-- atómica, pero valida usuario, rol, tenant y campaña antes de escribir.
create or replace function public.asignar_campana_dataset(
  p_dataset uuid,
...
begin
  ...
  if not coalesce(v_admin, false) then
    raise exception 'Sólo un administrador puede asignar campañas.';
  end if;

  if not exists (
    select 1 from public.dataset d
     where d.id = p_dataset and d.tenant_id = v_tenant
  ) then
    raise exception 'La base no existe o no pertenece a tu espacio.';
  end if;

  if p_campana is not null and not exists (
    select 1 from public.campana c
     where c.id = p_campana and c.tenant_id = v_tenant and c.activo
  ) then
    raise exception 'La campaña no existe, está inactiva o pertenece a otro espacio.';
  end if;

  update public.dataset
     set campana_id = p_campana
   where id = p_dataset and tenant_id = v_tenant;
  ...
  return jsonb_build_object(
    'datasetId', p_dataset,
    'campanaId', p_campana,
    'cargasActualizadas', v_cargas,
    'registrosActualizados', v_registros
  );
end;
$$;
...
revoke execute on function public.asignar_campana_dataset(uuid, uuid)
  from public, anon;
grant execute on function public.asignar_campana_dataset(uuid, uuid)
  to authenticated;
