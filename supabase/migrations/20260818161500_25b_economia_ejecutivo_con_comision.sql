    ...
    sum(
      coalesce(
        ...
        case when p.parentesco = 'titular' and p.edad is not null then
          (select t.valor_uf from tarifa t
            where t.agrupacion_meta = p.agrupacion_meta
              and t.criterio = 'edad' and t.alcance = 'titular'
        ...
        case when p.parentesco = 'carga' then
          (select t.valor_uf from tarifa t
            where t.agrupacion_meta = p.agrupacion_meta
              and t.criterio = 'edad' and t.alcance = 'adicional'
              and (p_campana is null or t.campana_id = p_campana)
              and t.vigencia_desde <= p_hasta
              and (t.vigencia_hasta is null or t.vigencia_hasta >= p_desde)
            order by t.desde desc limit 1)
...
-- Comisión escalonada y bonos del esquema real, sumados por ejecutivo.
comisiones as (
  select ejecutivo_id, sum(total_clp) as total
...
armado as (
  select
    ...
    round(
      coalesce(c.base, 0)
      + coalesce(i.asegurados, 0) * coalesce(c.comision_unitaria, 0)
      + coalesce(cm.total, 0), 0) as renta_bruta_clp,
    round(
      (coalesce(c.base, 0)
       + coalesce(i.asegurados, 0) * coalesce(c.comision_unitaria, 0)
       + coalesce(cm.total, 0)) * coalesce(c.factor_leyes, 1), 0) as costo_empresa_clp
  from ejecutivo e
  ...
  left join comisiones cm on cm.ejecutivo_id = e.id
  where e.activo
