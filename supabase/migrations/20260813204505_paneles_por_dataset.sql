...
-- Los paneles legacy de Seguros conservan dataset_id NULL; cada dataset
-- genérico obtiene un panel independiente por usuario.
...
-- Los widgets heredan visibilidad del panel, pero sólo el dueño (o un admin
-- en un panel compartido) puede modificarlos. La policy ALL anterior permitía
-- editar cualquier panel compartido visible del tenant.
...
declare
  ...
  v_nombre text;
  v_metrica uuid;
  v_metrica_nombre text;
  v_metrica_agregacion text;
  v_dimension uuid;
  v_dimension_nombre text;
  v_fecha uuid;
  v_fecha_nombre text;
begin
  ...
  if exists (select 1 from public.panel_widget pw where pw.panel_id = v_panel) then
    return v_panel;
  end if;
  ...
  insert into public.panel_widget
    (panel_id, tipo, titulo, config, x, y, w, h, orden)
  values
    (v_panel, 'kpi', 'Total de registros',
      jsonb_build_object('fuente','dataset','datasetId',p_dataset,'agregacion','count',
        'tieneFecha',v_fecha is not null),
      0, 0, 3, 4, 0);
  ...
  if v_metrica is not null then
    insert into public.panel_widget
      (panel_id, tipo, titulo, config, x, y, w, h, orden)
    values
      (v_panel, 'kpi', v_metrica_nombre,
        jsonb_build_object('fuente','dataset','datasetId',p_dataset,
          'metricaId',v_metrica,'agregacion',v_metrica_agregacion,
          'tieneFecha',v_fecha is not null),
        3, 0, 3, 4, 1);
  end if;
  ...
  if v_dimension is not null then
    ...
    values
      (v_panel, 'barras',
        coalesce(v_metrica_nombre, 'Registros') || ' por ' || lower(v_dimension_nombre),
  ...
  if v_fecha is not null then
    ...
    values
      (v_panel, 'area',
        coalesce(v_metrica_nombre, 'Registros') || ' por ' || lower(v_fecha_nombre),
        jsonb_strip_nulls(jsonb_build_object('fuente','dataset','datasetId',p_dataset,
