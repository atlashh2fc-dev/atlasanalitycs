...
create table kpi_ejecutivo (
  ...
  ip_d numeric(10,4), ip_c numeric(10,4), ip_v numeric(10,4),
  tasa_cierre numeric(10,4), profundidad numeric(10,4),
  uf_por_asegurado numeric(10,4), indice_venta_sana numeric(10,4),
  cuartil_ip_d smallint, cuartil_ip_c smallint, cuartil_ip_v smallint,
  ranking int, percentil numeric(5,2),
  meta_prorrateada numeric(12,2), cumplimiento numeric(10,4),
...
comment on column kpi_ejecutivo.ip_c is
  'Efectividad de cierre. Destapa el caso real de 404 cotizaciones con 6 contratos frente a 75 con 12.';
...
create table kpi_equipo (
  ...
  ejecutivos_activos int, dg_total int, cotizaciones int, contratos int,
  asegurados int, asegurados_netos int, uf numeric(14,4),
  meta numeric(12,2), cumplimiento numeric(10,4),
  ritmo_requerido numeric(10,4), proyeccion_cierre numeric(12,2),
  media_ip_d numeric(10,4), mediana_ip_d numeric(10,4),
  q1_ip_d numeric(10,4), q3_ip_d numeric(10,4),
  desviacion_ip_d numeric(10,4), coef_variacion numeric(10,4),
  pct_sobre_meta numeric(5,2), brecha_oportunidad numeric(12,2),
  calculado_at timestamptz not null default now(),
...
comment on column kpi_equipo.brecha_oportunidad is
  'Asegurados adicionales si los ejecutivos bajo la mediana la alcanzaran. Convierte el análisis en gestión.';
...
begin
  ...
  anu as (
    select v.ejecutivo_id, v.campana_id,
      ...
      from anulacion a join venta v on v.id = a.venta_id
     where a.tenant_id = v_tenant and a.periodo_descuento between v_inicio and v_fin
     group by 1,2
  ),
  consolidado as (
    select b.ejecutivo_id, b.campana_id,
           coalesce(d.dg,0) as dg, coalesce(c.cotizaciones,0) as cotizaciones,
  ...
  on conflict (periodo_id, ejecutivo_id, campana_id) do update set
    dg = excluded.dg, cotizaciones = excluded.cotizaciones,
    gestiones = excluded.gestiones, contratos = excluded.contratos,
    asegurados = excluded.asegurados, asegurados_anulados = excluded.asegurados_anulados,
    asegurados_netos = excluded.asegurados_netos, uf = excluded.uf,
    ip_d = excluded.ip_d, ip_c = excluded.ip_c, ip_v = excluded.ip_v,
    tasa_cierre = excluded.tasa_cierre, profundidad = excluded.profundidad,
    uf_por_asegurado = excluded.uf_por_asegurado,
    indice_venta_sana = excluded.indice_venta_sana,
    cuartil_ip_d = excluded.cuartil_ip_d, cuartil_ip_c = excluded.cuartil_ip_c,
    cuartil_ip_v = excluded.cuartil_ip_v, ranking = excluded.ranking,
    percentil = excluded.percentil, calculado_at = now();
...
with serie as (
  select k.tenant_id, k.ejecutivo_id, k.campana_id,
         p.id as periodo_id, p.tipo, p.etiqueta, p.fecha_inicio,
         k.ip_d, k.cuartil_ip_d,
   ...
   window w as (partition by k.ejecutivo_id, k.campana_id, p.tipo order by p.fecha_inicio)
)
select s.*,
...
comment on view v_movilidad_cuartil is
  'ntile(4) ordena ascendente: cuartil 4 = mejor, 1 = peor. Estable_bajo tres periodos seguidos es la alerta más accionable.';

create view v_matriz_transicion as
select tenant_id, campana_id, tipo, periodo_anterior,
       etiqueta as periodo_actual, cuartil_anterior,
       cuartil_ip_d as cuartil_actual, count(*)::int as ejecutivos
  from v_movilidad_cuartil
 where cuartil_anterior is not null
 group by 1,2,3,4,5,6,7;

create view v_tendencia_equipo as
select e.tenant_id, e.campana_id, e.agrupacion_meta, p.tipo, p.etiqueta,
       p.fecha_inicio, e.asegurados_netos, e.meta, e.cumplimiento,
       e.coef_variacion, e.brecha_oportunidad,
       e.asegurados_netos - lag(e.asegurados_netos)
         over (partition by e.campana_id, e.agrupacion_meta, p.tipo order by p.fecha_inicio)
         as delta_periodo_anterior
  from kpi_equipo e
